...
export function createTaskCard(text = "", status = "default", id = null, dayIndex) {
  ...
  saveBtn.addEventListener("click", async () => {
    ...
    if (saveBtn.dataset.mode === "save") {
      const taskText = (taskCard.querySelector("input") || { value: "" }).value.trim();
...
function attachAddTaskListeners() {
  ...
  freshBtns.forEach((btn, idx) => {
    ...
    btn.addEventListener("click", () => {
      ...
      if (!window.tasksByWeek[window.currentWeekKey]) window.tasksByWeek[window.currentWeekKey] = {};
      if (!window.tasksByWeek[window.currentWeekKey][dayIndex]) window.tasksByWeek[window.currentWeekKey][dayIndex] = [];
